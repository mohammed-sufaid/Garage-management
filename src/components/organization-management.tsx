...
export function OrganizationManagement() {
  ...
  useEffect(() => {
    const fetchOrganization = async () => {
      try {
        debugger
        setIsFetching(true)
        const response = await fetch(apiendpoints.organization)
        ...
        if (Array.isArray(data) && data.length > 0) {
          ...
          toast({
            title: "No Data Found",
            description: "No organization records were found in the database.",
          })
        }
      } catch (error) {
        console.error(error)
  ...
  // ✅ Save (you can later connect it to PUT API)
  const handleSave = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${apiendpoints.organization}?id=${formData.id}`,
        {
          method: "PUT",
          headers: {
  ...
  if (isFetching) {
    return <p className="text-center text-muted-foreground">Loading organization data...</p>
  }
